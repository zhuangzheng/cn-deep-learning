...
# __回答:__
# 
# 不合理。有时候用户可能在抓拍运动中的人，或者拍摄的时候手不稳，又或者是他人距离有点远。
# 
# 方法是，在训练集加入噪声，对部分样本做适度模糊化处理，又或者是增加训练集，加入一些运动中的人脸或者是距离稍远的人脸。
...
for n in range(100):
    dog_in_humanset.append(dog_detector(human_files_short[n]))
    dog_in_dogset.append(dog_detector(dog_files_short[n]))

dog_human_acc = np.sum(dog_in_humanset)
dog_dog_acc = np.sum(dog_in_dogset)

print("人脸数据集中检测出狗的概率：  ",dog_human_acc,'%')
print("狗数据集中检测出狗的概率:  ",dog_dog_acc,'%')
...
# 
# __回答:__ 
# 1.用了三个卷积层，四个池化层，四个激活层，一个全连接层和三个dropout层
# 考虑到输入数据有15，000个特征而训练集上每个类型的样本平均只有50个，利用卷积神经网络要比全连接的深度神经网络有巨大的优势，而卷积层数量越多，分类的效果就越好，但是考虑到计算能力，只能使用三个卷积层
...
### TODO: 定义你的网络架构
model.add(Conv2D(filters=16,input_shape = [224,224,3],kernel_size=5,strides=3,padding='same',activation='relu'))
model.add(MaxPooling2D(pool_size=(2,2),strides=2))
model.add(Dropout(0.8))

model.add(Conv2D(filters=32,kernel_size=5,strides=3,padding='same',activation='relu'))
model.add(MaxPooling2D(pool_size=(2,2),strides=2))
model.add(Dropout(0.8))
...
# __回答:__ 
# 
# 我搭建的模型是一个序贯模型，是在Resnet50模型后添加一个GAP层,降低输出的维度，有助于防止过拟合，再添加一个全连接层，有133个神经元，对应133个分类，再用softmax作为激活函数计算出最有可能的分类。

# In[34]:

### TODO: 定义你的框架
doggy=Sequential()
doggy.add(GlobalAveragePooling2D(input_shape=train_VGG19.shape[1:]))
doggy.add(Dense(512,activation='relu'))
doggy.add(Dense(512,activation='relu'))
doggy.add(Dense(133,activation='softmax'))
...
### TODO: 编译模型
doggy.compile(loss='categorical_crossentropy',optimizer='adam',metrics=['accuracy'])
...
doggy.fit(train_VGG19,train_targets,validation_data=(valid_VGG19,valid_targets),callbacks=[checkpointer],verbose=1,epochs=50,batch_size=32)


# In[62]:

### TODO: 加载具有最佳验证loss的模型权重
doggy.load_weights('saved_models/weights.best.VGG19.hdf5')
...
### TODO: 在测试集上计算分类准确率
VGG19_predictions = [np.argmax(doggy.predict(np.expand_dims(feature, axis=0))) for feature in test_VGG19]
...
def VGG19_predict_breed(img_path):
    ...
    predicted_vector = doggy.predict(bottleneck_feature)
    return dog_names[np.argmax(predicted_vector)]
...
def final_detector(img_path):
    print("\nHello!")
    plt.imshow(cv2.cvtColor(cv2.imread(img_path),cv.COLOR_BGR2RGB))
    plt.show()
    if  dog_detector(img_path):
        print('Wow! your dog looks cute,I guess it is a ...\n') 
        print( VGG19_predict_breed(img_path))
    elif face_detector(img_path):
        print('Human ,You look like a\nhold on...\n')
        print(VGG19_predict_breed(img_path))
    else:
        print('Nah...,there are something wrong,try another picture\n')
...
# 1. 输出结果比你预想的要好吗 :) ？或者更糟 :( ？
# 2. 提出至少三点改进你的模型的想法。
# 1.有好有坏。首先狗和狗的类型都被正确地检测出来了，猫也都被排除在外，比较意外的是我加入了三张狼的图片全都正确地被排除了；
#   但是人脸检测的部分就不行了，可以看到我放入的人脸照片里有两张没被检测出来，相反放入的两张娃娃照片都被检测成了人脸。
# 2.1）我的模型正确率不是很理想，训练集太小是一个原因，对于一个150,000左右特征的输入，每个类型只有50张左右的样本实在太少了，可以增加训练集数据，通过各种途径，雇人去拍、网上批量下载等方法
#   2）通过数据扩增，对现有数据集旋转，偏移，随机裁剪，倾斜，镜像等方法增加数据集
#   3）提高训练时间，增加迭代数目

# In[93]:

## TODO: 在你的电脑上，在步骤6中，至少在6张图片上运行你的算法。
for file_name in np.array(glob('testImage/*')):
    final_detector(file_name)
## 自由地使用所需的代码单元数吧
